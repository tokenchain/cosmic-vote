...
const actions = {
    getVotingPowers: async ({ commit }, { blockTag, token, addresses }) => {
        ...
        try {
            ...
            const votingPowers = Object.fromEntries(addresses.map(address => [address, 0]));
            if (result && result.poolShares) {
                result.poolShares.forEach(poolShare => poolShare.poolId.tokens.map(poolToken => {
                    ...
                    if (tokenAddress === token.toLowerCase()) {
                        ...
                        const shares = (poolToken.balance / poolShare.poolId.totalShares) *
                            poolShare.balance;
    ...
    getVotingPowersByPools: async ({ commit }, { blockTag, token, addresses }) => {
        commit('GET_VOTING_POWERS_REQUEST');
        try {
            const block = blockTag === 'latest' ? undefined : { number: blockTag };
            const result = await request('getVotingPowers', {
            ...
            const votingPowers = Object.fromEntries(addresses.map(address => [address, {}]));
            if (result && result.poolShares) {
                result.poolShares.forEach(poolShare => poolShare.poolId.tokens.map(poolToken => {
