...
const actions = {
    send: async ({ commit, dispatch, rootState }, { token, type, payload }) => {
        ...
        try {
            ...
            const errorMessage = e && e.error_description
    ...
    getProposals: async ({ commit, dispatch, rootState }, payload) => {
        ...
        try {
            let proposals = await client.request(`${payload}/proposals`);
            if (proposals) {
                let balances = await dispatch('multicall', {
                    name: 'TestToken',
                    calls: Object.values(proposals).map((proposal) => {
                        return [proposal.msg.token, 'balanceOf', [proposal.address]];
                    })
                });
                balances = balances.map(balance => parseFloat(formatUnits(balance.toString(), decimals)));
                proposals = Object.fromEntries(Object.entries(proposals).map((proposal, i) => {
    ...
    getProposal: async ({ commit, dispatch, rootState }, payload) => {
        ...
        try {
            ...
            const blockTag = snapshot > rootState.web3.blockNumber ? 'latest' : parseInt(snapshot);
            const votersBalances = await dispatch('getVotersBalances', {
                token: payload.token,
                addresses: Object.values(result.votes).map((vote) => vote.address),
                blockTag
            });
            ...
            result.votes = Object.fromEntries(Object.entries(result.votes)
                .map((vote) => {
            ...
            result.results = {
                totalVotes: result.proposal.msg.payload.choices.map((choice, i) => Object.values(result.votes).filter((vote) => vote.msg.payload.choice === i + 1).length),
                totalBalances: result.proposal.msg.payload.choices.map((choice, i) => Object.values(result.votes)
                    .filter((vote) => vote.msg.payload.choice === i + 1)
                    .reduce((a, b) => a + b.balance, 0)),
                totalBptBalances: bptDisabled
                    ? 0
                    : result.proposal.msg.payload.choices.map((choice, i) => Object.values(result.votes)
                        .filter((vote) => vote.msg.payload.choice === i + 1)
                        .reduce((a, b) => a + b.bptBalance, 0)),
                totalWalletBalances: result.proposal.msg.payload.choices.map((choice, i) => Object.values(result.votes)
                    .filter((vote) => vote.msg.payload.choice === i + 1)
                    .reduce((a, b) => a + b.walletBalance, 0)),
    ...
    getVotersBalances: async ({ commit, rootState }, { token, addresses, blockTag }) => {
        ...
        try {
            ...
            response.forEach((value, i) => {
                balances[addresses[i]] = parseFloat(formatUnits(value.toString(), decimals));
            });
    ...
    getPower: async ({ commit, dispatch, rootState }, { token, address, snapshot }) => {
        ...
        try {
            ...
            const bpt = Object.values(bpts[address]).reduce((a, b) => a + b, 0);
