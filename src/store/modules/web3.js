...
const actions = {
    ...
    multicall: async ({ commit }, { name, calls, options }) => {
        ...
        try {
            let [, response] = await multi.aggregate(calls.map(call => [
                call[0].toLowerCase(),
                itf.encodeFunctionData(call[1], call[2])
            ]), options || {});
            response = response.map((call, i) => itf.decodeFunctionResult(calls[i][1], call));
    ...
    metadata: async ({ commit, dispatch }) => {
        try {
            const response = await dispatch('multicall', {
                name: 'TestToken',
                calls: Object.values(namespaces).map((namespace) => [
                    namespace.address,
                    'decimals',
                    []
                ])
            });
            const payload = Object.fromEntries(response.map((item, i) => [
